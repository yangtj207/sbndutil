void opdet_digi_dat2root() {
  // Script to create root files with test bench data for the digitisation of PMTs and ARAPUCAs
  // It produces digi_arapuca_sbnd.root and digi_pmt_sbnd.root, which we put on sbnd_data.
  // Author: Iker de Icaza icaza@fnal.gov

  TString dir = gSystem->UnixPathName(__FILE__);
  dir.ReplaceAll("opdet_digi_dat2root.C","");
  dir.ReplaceAll("/./","/");

   TFile *f_arapuca = new TFile("digi_arapuca_sbnd.root","RECREATE", "Test bench data for SBND ARAPUCAS", 0); 

   TH1D* TimeArapucaVUV = new TH1D("TimeArapucaVUV", "Time Profile ArapucaVUV", 150, 0.0, 150.0);//histogram that stores the arrival time of photons at SiPM (t=0 is the time is reaches the outside of the optical window) for arapuca T1
    ...
    for(size_t i = 1; i <= 150; i++)TimeArapucaVUV->SetBinContent(i, x[i-1]);
    TimeArapucaVUV->Write();

    TH1D* TimeArapucaVIS = new TH1D("TimeArapucaVIS", "Time Profile ArapucaVIS", 90, 0.0, 90.0);//histogram that stores the arrival time of photons at SiPM (t=0 is the time is reaches the outside of the optical window) for arapuca T2
    ...
    for(size_t i = 1; i <= 90; i++)TimeArapucaVIS->SetBinContent(i, x2[i-1]);
    TimeArapucaVIS->Write();

    TH1D* TimeXArapucaVUV = new TH1D("TimeXArapucaVUV", "Time Profile XArapucaVUV", 20, 0.0, 20.0);//histogram that stores the arrival time of photons at SiPM (t=0 is the time is reaches the outside of the optical window) for X-arapuca
