...
def getmetadata(inputfile):
    ...
    if local != '':
        proc = subprocess.Popen(
            ["sam_metadata_dumper", "-H", local], stdout=subprocess.PIPE)
    else:
        url = project_utilities.path_to_url(inputfile)
        proc = subprocess.Popen(
            ["sam_metadata_dumper", "-H", url], stdout=subprocess.PIPE)
    ...
    for line in lines:
        ...
        if c >= 4 and c <= num_lines-2:
            if columns[1] == 'dataTier':
                ...
                md['end_time'] = str(
                    E[0])+'-'+str(E[1])+'-'+str(E[2])+'T'+str(E[3])+':'+str(E[4])+':'+str(E[5])
            elif columns[1] == 'startTime':
                S = time.localtime(int(columns[-1]))
                md['start_time'] = str(
                    S[0])+'-'+str(S[1])+'-'+str(S[2])+'T'+str(S[3])+':'+str(S[4])+':'+str(S[5])
    ...
    md['application'] = {'family': app_family,
